<div class="container">
    <div class="main_panel content radius">
        ...
        <div class="j_content_message_sync"></div>

        <form method="post" class="form_os" name="sync_os">
            <div class='form_os_row_center'>
                <button class="radius sync">Sincronizar Os's</button>
            </div>
        </form>

        <form method="post" class="form_os" name="sync_note">
            <div class='form_os_row_center' style="margin-top: 20px;">
                <button class="radius sync">Sincronizar Notas</button>
            </div>
        </form>
...
<div class="container">
        ...
        <form method="post" class="form_os">
            ...
            <div class="form_os_row">
                ...
                <select name="status" class="radius">
                    <option value="" <?= (!empty($order_status) && $order_status == 1 ? 'selected=selected' : null); ?>>Aberto</option>
                    <option value="" <?= (!empty($order_status) && $order_status == 2 ? 'selected=selected' : null); ?>>Fechado</option>
                </select>
            </div>
            <div class="form_os_row">
                Técnico:
    ...
    <div class="main_panel content radius">
        ...
        if ($url[0] != 'index') {
            ?>
            <form method="post" class="form_os" name="send_note">
                <input type="hidden" value="<?= $order_cod; ?>" name="order_cod">
                <div class="form_os_row">
                    Nota:
                    <textarea name="text"></textarea>
                </div>
                <div class='form_os_row_reverse'>
                    <button type='submit' class='radius add_os'>Cadastrar Nota</button>
                </div>
            </form>
